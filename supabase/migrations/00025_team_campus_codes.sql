...
--
-- ── Uniqueness ───────────────────────────────────────────────────────────
-- A code only has to be unique within its parent (team codes within a campus,
-- campus codes within an org), and only when set — hence partial unique indexes
-- on the non-null rows rather than NOT NULL columns.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Unique per parent, only for rows that have a code set.
create unique index if not exists idx_campuses_code
  on campuses (organization_id, upper(code))
